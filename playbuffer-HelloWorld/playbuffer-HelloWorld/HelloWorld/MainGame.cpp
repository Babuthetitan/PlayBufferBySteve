...
bool MainGameUpdate( float elapsedTime )
{
	...
	Play::DrawFontText("132px", "YOUR SCORE:" + std::to_string(gameState.score), { DISPLAY_WIDTH / 2, 50 }, Play::CENTRE );
...
void UpdateFan()
{
	//I have reduced the probabilities of drivers & spanner spawns to a 1% chance out of 100
	//This is possible due to the random roll happening 60 times a second
...
void UpdateDestroyed()
{
	...
	for (int id_dead : vDead)
	{
		...
		if (obj_dead.frame % 2)
			Play::UpdateGameObject(obj_dead, (10 - obj_dead.frame) / 10.0f);
